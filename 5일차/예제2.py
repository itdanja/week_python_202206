...
def countDown( n ) :
    if n == 0 :
        ...
        print( n , "초" )
        time.sleep( 1 ) # time.sleep( 초 ) : 해당 초 만큼 일시중지
        countDown( n-1 )
countDown( 3 )
print("----------------------------------------------------------")
#재귀함수 예시3 : 별찍기
...
for num in range( 1 , 10 ) :
    ...
    print( )
"""
    print 에서 사용되는 형식(모양=포멧)문자 = 서식문자 = 함수.format() 
        print( "형식문자1 형식문자2" % 값1 , 값2 ) 
        %d  10진수        %2d : 2칸 자리 차지 [ 해당 자릿수에 수가 없으면 공백 ]  
        %o  8진수         %02d : 2칸 자리 차지 [ 해당 자릿수에 수가 없으면 O 으로 대입 ] 
        %x  16진수 
        %f  실수 
        %c  단일 문자 
        %s  문자열 
